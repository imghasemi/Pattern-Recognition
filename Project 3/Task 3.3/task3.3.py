...
# PCA Algorithm
# -------------------------------------------------------------------------------------------------------------
# Calculating PCA Algorithm with two args: Data, integer length of eigenVector (Biggest ones)
...
# LDA Algorithm
# -------------------------------------------------------------------------------------------------------------
# Then we start to calculate the Linear Discriminant Analysis (LDA) for reducing the dimension from 500 to 2s
def LDA(X, classLength, nBiggestVector):
    ...
    # meanList: we use this array (150 elements) for sake of simplicity during the calculation
    meanList = np.concatenate([[classMeans[i]] * classLength[i] for i in range(len(classLength))])
...
if __name__ == "__main__":
    ...
    axs = Axes3D(fig)
    for i, j in zip([1., 2., 3.], ['green', 'blue', 'red']):
        axs.scatter3D((finalPCA_3D.T)[0][np.where(Y_Label == i)], (finalPCA_3D.T)[1][np.where(Y_Label == i)],
    ...
    axs = Axes3D(fig)
    for i, j in zip([1., 2., 3.], ['green', 'blue', 'red']):
        axs.scatter3D((finalLDA_3D.T)[0][np.where(Y_Label == i)], (finalLDA_3D.T)[1][np.where(Y_Label == i)],
